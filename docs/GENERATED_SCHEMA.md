...
uncertainties and lower confidence bounds, conditional band transmittances,
their conservative UCB, `film_proxy_pass`, loading scale, and `spectrum_json`.
MOST fields contain ΔH, Wh/kg, `specific_energy_lcb`, log/linear half-life at 305 K,
...
`joint_uv_pass` includes the conditional film proxy; `most_pass` includes the
specific-energy threshold. `safety_pass` and `joint_pass` are transparent
